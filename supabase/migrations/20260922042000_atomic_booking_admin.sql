/*
  Atomic booking, schedule save, catalog administration, and payment review.
  Prices, duration, and payment state are assigned in the database.
*/

DROP FUNCTION IF EXISTS public.create_booking_atomic(text, text, text, text, text, text, text, text, text, text);
...
AS $$
  SELECT jsonb_build_object(
    ...
    'payment_expires_at', p.payment_expires_at
  );
$$;
...
BEGIN
  ...
  ELSE
    ...
    v_appointment := OLD.appointment_start IS DISTINCT FROM NEW.appointment_start
      OR OLD.appointment_end IS DISTINCT FROM NEW.appointment_end;
    ...
    v_comments := OLD.comments IS DISTINCT FROM NEW.comments
      OR OLD.problem_description IS DISTINCT FROM NEW.problem_description;
    v_notes := OLD.technician_notes IS DISTINCT FROM NEW.technician_notes;
    v_payment := OLD.payment_status IS DISTINCT FROM NEW.payment_status;
    v_price := OLD.total_amount IS DISTINCT FROM NEW.total_amount OR OLD.subtotal IS DISTINCT FROM NEW.subtotal;
    v_customer := OLD.email IS DISTINCT FROM NEW.email
    ...
    v_changes :=
      ...
      (CASE WHEN v_price THEN 1 ELSE 0 END) +
      (CASE WHEN v_customer THEN 1 ELSE 0 END) +
      (CASE WHEN OLD.service_mode IS DISTINCT FROM NEW.service_mode THEN 1 ELSE 0 END) +
      (CASE WHEN OLD.device_type IS DISTINCT FROM NEW.device_type THEN 1 ELSE 0 END) +
      (CASE WHEN OLD.device_brand IS DISTINCT FROM NEW.device_brand THEN 1 ELSE 0 END) +
      (CASE WHEN OLD.device_model IS DISTINCT FROM NEW.device_model THEN 1 ELSE 0 END) +
    ...
    ELSIF v_changes = 1 AND v_payment THEN
      v_action := 'payment_changed';
    ELSIF v_changes = 1 AND v_price THEN
      v_action := 'price_changed';
    ELSIF v_changes = 1 AND v_customer THEN
      v_action := 'customer_changed';
    ELSE
      v_action := 'updated';
    END IF;
  END IF;
